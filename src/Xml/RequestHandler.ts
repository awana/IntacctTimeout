...
export default class RequestHandler {
  ...
  public async executeOnline(content: IFunction[]): Promise<OnlineResponse> {
    if (
      this.requestConfig.policyId != null &&
      this.requestConfig.policyId !== ""
    ) {
      this.requestConfig.policyId = "";
    }

    const request = new RequestBlock(
      this.clientConfig,
      this.requestConfig,
      content
    );
  ...
  private async execute(xml: string): Promise<RequestResponse> {
      ...
      try {
        ...
        if (error instanceof StatusCodeError) {
          if (error.statusCode >= 500 && error.statusCode <= 599) {
            if (
              this.requestConfig.noRetryServerErrorCodes.indexOf(
                error.statusCode
              ) !== -1
            ) {
    ...
    throw new Error(
      "Request retry count exceeded max retry count: " +
